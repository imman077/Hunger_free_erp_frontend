...
export const UserGrowthChartCard: React.FC = () => {
  return (
    <div
      className="group relative rounded-xl p-5 md:p-7 border transition-all duration-500"
      style={{
...
export const DashboardMainField: React.FC = () => {
  const { data, isLoading } = useDashboardStore();
  const { stats } = data;

  return (
    <div className="w-full space-y-8 relative flex-1 overflow-y-auto no-scrollbar">
      {isLoading && (
        <div className="absolute inset-0 bg-black/5 flex items-center justify-center z-50 rounded-xl">
