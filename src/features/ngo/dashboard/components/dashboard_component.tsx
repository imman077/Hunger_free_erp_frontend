import { Trophy, TrendingUp, LayoutGrid, ArrowUpRight, Activity, Package, Zap } from "lucide-react";

interface WelcomeHeaderProps {
  progressToNext?: number;
}

export const WelcomeHeader = ({ progressToNext = 75 }: WelcomeHeaderProps) => {
  return (
    <div
...
export const UpdateFeed = ({ notifications }: UpdateFeedProps) => {
  return (
    <div
                ...
                <div className="flex items-center gap-3 min-w-0">
                  <div
                    className={`w-10 h-10 rounded-xl shrink-0 flex items-center justify-center transition-all duration-300 group-hover:scale-105 border ${
                      isDonation
                        ? "bg-hf-green/10 text-hf-green border-hf-green/20"
                        : "bg-blue-500/10 text-blue-500 border-blue-550/20"
                    }`}
                    style={{
                      borderColor: isDonation ? "" : "var(--border-color)"
                    }}
                  >
                    {isDonation ? (
                      <Package size={18} />
